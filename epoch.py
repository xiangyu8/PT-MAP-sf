...
if __name__ == '__main__':
    ...
    """
    if params.save_iter != -1:
        outfile = os.path.join( checkpoint_dir.replace("checkpoints","features"), split + "_" + str(params.save_iter)+ "_epoch.hdf5") 
    else:
        outfile = os.path.join( checkpoint_dir.replace("checkpoints","features"), split + "_epoch.hdf5") 
    ...
    if params.method == 'manifold_mixup':
        ...
        if params.dataset == 'cifar':
            ...
            model = wrn_mixup_model.wrn28_10(200)
 #   elif params.method == 'dct':
 #       model = ResNetDCT_Upscaled_Static(channels= 24)
    else:
    ...
    if params.method == 'manifold_mixup' or params.method == 'S2M2_R' :
        ...
        state = tmp['state']
        epoch_best = tmp['epoch']
        print("the best model is from epoch: ", epoch_best)
        """
