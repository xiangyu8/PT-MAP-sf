...
def train_dct(base_loader, base_loader_test, val_loader ,model, start_epoch, stop_epoch, params,tmp):
    ...
    for epoch in range(start_epoch, stop_epoch):
        ...
        for batch_idx, (input_var, target_var) in enumerate(base_loader):
           ...
           # print("inside train_dct: ", outputs[0].shape,outputs[1].shape ,type(input_dct_var))
        ...
        for i, x in enumerate(loader):
           ...
           # print("shape of x: ",x.shape)
            x = x.view(-1,24,image_size_dct,image_size_dct)
            if use_gpu:
                x = x.cuda()
...
def train_plain(base_loader, base_loader_test, val_loader ,model, start_epoch, stop_epoch, params,tmp):

    val_acc_best = 0.0
    if path.exists('./valplain_'+ params.dataset +  '.pt'):
        loader = torch.load('./valplain_'+ params.dataset +  '.pt')
    else:
        loader = []
        for ii , (x,_) in enumerate(val_loader):
            loader.append(x)
          #  print("head of train_dct: ", x.shape)
        torch.save(loader,'./valplain_'+ params.dataset +  '.pt')
...
if __name__ == '__main__':
    ...
    elif params.method =='dct' or params.method =='plain':
        model = BaselineTrain( model_dict[params.model], params.num_classes, loss_type = 'dist')        
    ...
    if params.method =='dct':
        ...
        optimization = 'Adam'
        model = train_dct(base_loader, base_loader_test, val_loader,  model, start_epoch, start_epoch+stop_epoch, params,{})
    ...
    if params.method =='plain':
        ...
        cudnn.benchmark = True
        optimization = 'Adam'
        model = train_plain(base_loader, base_loader_test, val_loader,  model, start_epoch, start_epoch+stop_epoch, params,{})
